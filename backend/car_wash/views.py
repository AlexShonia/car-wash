...
def choose_time(request):
    ...
    starting_time = "8:30"
    current_hour = int(starting_time.split(":")[0])
    current_min = int(starting_time.split(":")[1])
    minute_increment = 30
    ...
    while current_hour <= 17:
        ...
        time_block = str(current_hour) + ":" + str(current_min)
        if Appointment.objects.filter(date=date + " " + time_block).exists():
